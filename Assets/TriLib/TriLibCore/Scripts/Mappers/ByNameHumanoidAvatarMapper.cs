...
namespace TriLibCore.Mappers
{
    ...
    public class ByNameHumanoidAvatarMapper : HumanoidAvatarMapper
    {
        ...
        /// <inheritdoc />
        /// <remarks>
        /// This method iterates through each <see cref="BoneMapping"/> in <see cref="BonesMapping"/> and checks each of its 
        /// candidate bone names. For each candidate, it clears a local list, then recursively searches the 
        /// <see cref="AssetLoaderContext.RootGameObject"/>'s transform hierarchy for matches.
        /// If a matching transform is found and it corresponds to an actual bone (indicated by <see cref="IModel.IsBone"/>),
        /// the mapping is added. If a required bone is not found, a warning is logged (if enabled) and an empty mapping is returned.
        /// </remarks>
        /// <param name="assetLoaderContext">
        /// The context containing the loaded model and relevant settings.
        /// </param>
        /// <returns>
        /// A <see cref="Dictionary{BoneMapping, Transform}"/> mapping each human bone (via <see cref="BoneMapping"/>)
        /// to the corresponding <see cref="Transform"/> from the model hierarchy.
        /// </returns>
