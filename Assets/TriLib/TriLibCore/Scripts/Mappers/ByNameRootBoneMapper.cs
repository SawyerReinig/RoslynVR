...
namespace TriLibCore.Mappers
{
    ...
    public class ByNameRootBoneMapper : RootBoneMapper
    {
        ...
        /// <inheritdoc />
        /// <remarks>
        /// This method iterates over the <see cref="RootBoneNames"/> array and, for each candidate name,
        /// calls <see cref="FindDeepChild(IList{Transform}, string)"/> to determine if any of the provided
        /// <paramref name="bones"/> have a name that matches (according to the specified string comparison mode).
        /// If a match is found, that <see cref="Transform"/> is immediately returned. If no candidate matches,
        /// the base mapper's implementation is used as a fallback.
        /// </remarks>
        /// <param name="assetLoaderContext">
        /// The context used for loading the model, containing references and additional settings.
        /// </param>
        /// <param name="bones">
        /// A list of candidate bone transforms extracted from the loaded model's hierarchy.
        /// </param>
        /// <returns>
        /// The <see cref="Transform"/> corresponding to the matched root bone name, or if no match is found, 
