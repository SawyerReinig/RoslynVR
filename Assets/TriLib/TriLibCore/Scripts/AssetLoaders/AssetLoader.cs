...
#if UNITY_WEBGL
    // Advise coroutines on WebGL to avoid browser 'unresponsive' messages
    if (!assetLoaderOptions.UseCoroutines)
    {
        Debug.LogWarning(
            "Please consider enabling 'AssetLoaderOptions.UseCoroutines'. " +
            "This can prevent the browser from reporting 'The page is not responding'."
        );
        Debug.LogWarning(ValidationMessage);
    }
#endif
